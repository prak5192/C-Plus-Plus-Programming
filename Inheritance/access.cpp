...
private:
    int m_nPrivate; // can only be accessed by Base 
                    // member functions (but not derived classes)
protected:
    int m_nProtected; // can be accessed by Base member 
...
public:
    Derived()
    {
        ...
        m_nPublic = 1; // allowed: can access public base members 
                       // from derived class
...
int main()
{
    ...
    cBase.m_nPublic = 1; // allowed: can access public members 
                         // from outside class
//    cBase.m_nPrivate = 2; // not allowed: can not access private 
